...
1. Fold each trajectory's `harness effect`, `advisory`, `dispositions`, `workers`, `usage`, `memory consulted`, `friction`, and `user choices` lines (older records: `subagents` and `advisor decisions` blocks instead; `unknown` is a gap, never a zero) into a page matched on **identity** (same component; same task type × dial; same memory file; same stage) — never on wording; create a page only when none matches.
...
3. A page with ≥ 3 runs and a clear direction becomes `supported` with one `fix:` — one atomic change (a dial default, an agent-definition line, a `tasks.md` row, a README "worth reading when" line, a `routes.md` line). A page later contradicted becomes `refuted`. Friction that recurs across records is one page, not one per wording; its proposed fix stays `proposed` in the page until a record's `verification` line supports it.
