...
- Keep files short: `harness.md` is the core (≈ 1.2k tokens, every byte re-sent on every call), each `SKILL.md` ≤ ~2.5k, satellites (`advisory.md`, `tasks.md`, …) ≤ ~1.5k. Every rule traces to a real failure; delete rules that stop earning their tokens.
