...
## Claude Code
- Standalone install: `/mini-harness on|off|status|init|loop|wiki|gui` or `/mini-harness <request>`; plugin install: the same as `/mini-harness:mini-harness …`. Direct skills: `/mh-init`, `/mh-loop`, `/mh-wiki`, `/mh-eval`, `/i-have-adhd`.
