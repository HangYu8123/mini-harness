...
Read by `mh-loop`. The loop's counters, gates, and durable record; bounded-wait rules live in `stay_active.md`. Each pass runs the way the native flow would (delegated or direct — the platform decides); the loop keeps the ledger.
...
Modulates only *how the body searches* — never the criteria, exit set, caps, or write-guard.
